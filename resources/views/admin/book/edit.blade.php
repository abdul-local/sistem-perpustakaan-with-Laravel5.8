...
@section('content')
    <div class="box">
        ...
        <form action="{{route('admin/book/update',$book)}}" method="POST" enctype="multipart/form-data">
                ...
                <input type="text" name="title" class="form-control" placeholder="Masukan Judul Buku" value="{{$book->title ?? old('title')}}">
                </div>
                @error('title')
                ...
                <div class="form-group @error('description') has-error @enderror ">
                    ...
                    <textarea name="description" id="" rows="3" class="form-control" placeholder="Masukan Deskripsi Buku" 
                    value="{{$book->description ?? old('description')}}" >{{$book->description ?? old('description')}}</textarea>
                </div>
                @error('description')
                <span class="help-block">{{$message}}</span>   
                @enderror
